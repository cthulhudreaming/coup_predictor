...
coup_data <- join_copo %>%
  group_by(polity, realized_coup) %>% 
  count(realized_coup) %>% 
  mutate(number = n)


coup_sample <- coup_data %>% 
  rep_sample_n(size = nrow(coup_data), reps = 1000, replace = TRUE) %>% 
  group_by(replicate, polity) %>% 
...
ggplot(coup_sample, aes(x = polity, y = sum)) + 
  geom_point() +
