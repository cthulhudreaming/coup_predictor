...
regime_coups <- full_join(coups_regime, polity_regime, 
                         by = c("cow_code" = "ccode",
                                "country_where_action_occurred" = "country",
                                "year_of_event" = "year"))
regime_coups <- regime_coups %>% 
  filter(polity != -88) %>% 
  filter(polity != -77) %>% 
  filter(polity != -66)

not_clean_countries <- countries %>% 
  mutate(control = case_when(
    str_detect(government_type, "absolute monarchy") ~ '3',
    str_detect(government_type, "communist") ~ '3',
    str_detect(government_type, "semi-presidential") ~ '2',
...
regime_full <- full_join(regime_coups, not_clean_countries, 
                         by = c("country_where_action_occurred" = "country")) %>% 
  filter(!is.na(realized_coup)) %>% 
  filter(!is.na(polity))
